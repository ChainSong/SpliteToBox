...
namespace AllocationPickTask.Common
{
    public class SnowFlakeHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string NextRedisId_PL()
        {
            lock (this)
            {
                long NewNum;
                RedisCacheHelper.IncrementValue("SDCCreatePL", out NewNum);
                return "1078" + NewNum.ToString().PadLeft(6, '0') + new Random().Next(1000, 9999);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string NextRedisId_CPOD(string NumStr)
        {
            lock (this)
            {
                long NewNum;
                RedisCacheHelper.IncrementValue("SDCCreateCPOD", out NewNum);
                return DateTime.Now.ToString("yyMMdd") + NumStr + NewNum.ToString().PadLeft(4, '0');
            }
        }
        /// <summary>
        /// 获得下一个ID
        /// </summary>
        /// <returns></returns>
        public string NextRedisId_CPODPrint_No()
        {
            lock (this)
            {
                long NewNum;
                RedisCacheHelper.IncrementValue("SDCCreateCPOD_Print_No", out NewNum);
                return NewNum.ToString().PadLeft(4, '0');
            }
        }
        /// <summary>
        /// 解析雪花ID
